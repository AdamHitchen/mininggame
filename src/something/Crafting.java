...
public class Crafting {
    ...
    public Crafting(Game gm, Inventory inv) throws SlickException {
        ...
        recipes = xml.readConfig(Inventory.currentDirectory + "/res/recipes.xml");
    ...
    public void update(GameContainer gc) {
        ...
        if (uiOpen) {
            for (int i = 0; i < buttons.size(); i++) {
                CraftingButton button = buttons.get(i);
    ...
    public void displayRecipes() {
        for (int i = 0; i < recipes.size(); i++) {
            ...
            List mats = recipe.returnMaterials();
            List numMats = recipe.returnNumMats();
